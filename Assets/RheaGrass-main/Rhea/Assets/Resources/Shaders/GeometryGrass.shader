...
#endif
		// Input must be 'triangle' (not 'point') because tessellation outputs triangle topology.
		// We use the centroid of the triangle for grass blade placement.
		void geom(triangle VertexOutput input[3], inout TriangleStream<GeometryData> triStream)
		{
			// Compute centroid of the tessellated triangle for blade placement
			VertexOutput v;
			v.vertex  = (input[0].vertex  + input[1].vertex  + input[2].vertex)  / 3.0f;
			v.normal  = normalize(input[0].normal  + input[1].normal  + input[2].normal);
			v.tangent = (input[0].tangent  + input[1].tangent  + input[2].tangent)  / 3.0f;
			v.uv      = (input[0].uv       + input[1].uv       + input[2].uv)       / 3.0f;
			v.color   = (input[0].color    + input[1].color    + input[2].color)    / 3.0f;
			//Sample the trimming map first to determine if grass will be spawned at this vertex at all
			float trimming = tex2Dlod(_GrassMap, float4(v.uv, 0, 0)).r;
			if (trimming >= _GrassThreshold)
			{
				...
				//Initialize positional and directional data (using centroid vertex)
